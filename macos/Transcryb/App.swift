...
struct TranscrybApp: App {
    ...
    var body: some Scene {
        ...
        // Menu bar item
        MenuBarExtra("Transcryb", systemImage: "microphone.fill") {
            VStack(spacing: 8) {
                Button(action: {
                    NSApp.activate(ignoringOtherApps: true)
                ...
                Button(action: {
                    ...
                    Label("Quit Transcryb", systemImage: "power")
                }
            }
            .padding(8)
            .onAppear {
...
extension TranscrybApp {
    ...
    private func showModelNotLoadedAlert() {
        ...
        alert.informativeText = "Please download and select a transcription model in Settings before using the recording hotkey."
